...
<body>
    ...
    <script>
        ...
        function renderCourses() {
            document.getElementById('courses-list').innerHTML = courses.length ? courses.map(c => {
                ...
                return '<div class="item-card" onclick="editCourse(\'' + c.id + '\')">' +
                    ...
                    '<div class="info"><h5>' + (c.name || c.subject || '未命名') + '</h5><span>班級 ' + c.classCode + ' · 週' + dayName + ' 第' + period + '節 · ' + (c.room || '教室未設定') + '</span></div></div>';
        ...
        function editCourse(id) {
            ...
            document.getElementById('edit-course-radius').value = currentCourse.radius || 100;
        ...
        async function saveCourse() {
            ...
            const radius = parseInt(document.getElementById('edit-course-radius').value) || 100;
            const p = ((sem.periods && sem.periods.length > 0 ? sem.periods : PERIODS[sem.div || "day"])).find(x => x.n == period) || {};
            console.log('儲存課程:', currentCourse.id, { subject, classCode, day, period, room, lat, lon, radius });
        ...
        function getLocationForEdit() {
            if (!navigator.geolocation) { toast('瀏覽器不支援定位', 'danger'); return; }
            toast('定位中...', 'warning');
            navigator.geolocation.getCurrentPosition(pos => {
                document.getElementById('edit-course-lat').value = pos.coords.latitude.toFixed(6);
                document.getElementById('edit-course-lon').value = pos.coords.longitude.toFixed(6);
                toast('已取得位置！精度約 ' + Math.round(pos.coords.accuracy) + ' 公尺');
            }, err => {
                toast('定位失敗: ' + err.message, 'danger');
        ...
        function getLocation() {
            ...
            navigator.geolocation.getCurrentPosition(pos => {
                ...
                toast('定位成功！');
            }, () => toast('定位失敗', 'danger'), { enableHighAccuracy: true });
        }
        ...
        async function addStudent() {
            ...
            if (!studentId || !name || !classCode) { toast('請填寫學號、姓名和班級', 'danger'); return; }
            const r = await api('/api/students', { method: 'POST', body: JSON.stringify({ studentId, name, classCode, phone, parentPhone }) });
            if (r?.success) { toast('學生已新增！'); closeModal('addStudent'); loadStudents(); document.getElementById('student-id').value = ''; document.getElementById('student-name').value = ''; document.getElementById('student-phone').value = ''; document.getElementById('student-parent-phone').value = ''; }
            else { toast(r?.message || '新增失敗', 'danger'); }
        }
